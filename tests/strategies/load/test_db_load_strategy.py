...
def target_table(target_address: str):
    ...
    with engine.connect() as conn:
        conn.execute(
            text("CREATE TABLE test_table (id INTEGER PRIMARY KEY, test_data TEXT)")
        )
        conn.commit()
    return engine
...
def test_db_load_strategy_success(target_address: str, target_table: Engine):
    ...
    data = TransformedData(content=json.dumps(rows).encode())
    ...
    with target_table.connect() as conn:
        result = conn.execute(
            text("SELECT id, test_data FROM test_table ORDER BY id")
        ).fetchall()
